...
import { DownOutlined, UserOutlined, LogoutOutlined, ShoppingCartOutlined, SearchOutlined, LoginOutlined } from '@ant-design/icons';
import { Dropdown, Space, message, Typography, Button, Badge, Tooltip } from 'antd';
...
const Navbar = () => {
  ...
  const fetchUserCartData = async () => {
    let cartId = ""
    await axios.get(`http://localhost:3344/carts/${currentUserId}`)
      .then((res) => {
        ...
        <span className={styles.userSection}>
          ...
          {currentUser
            ...
            <>
              <a href="/cart">
                <Badge count={userCart.length} showZero={true} title="">
                  <button className={styles.iconButton}>
                    <ShoppingCartOutlined style={{ color: '#FFF', fontSize: '180%' }} />
                  </button>
                </Badge>
              </a>
              <button className={styles.iconButton}>
            ...
            <Tooltip title="Sign in">
              <button className={styles.logButton} onClick={() => { navigate('/signin') }}>
