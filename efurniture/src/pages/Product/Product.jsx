...
import { Flex, Image, Typography, InputNumber, Button, Divider, Skeleton, Modal } from 'antd';
import { HeartOutlined, ShoppingCartOutlined, ShoppingOutlined, CheckCircleOutlined, MinusOutlined, PlusOutlined } from '@ant-design/icons';
...
export default function Product() {
  ...
  const addToCartForm = useFormik({
    ...
    onSubmit: (values) => {
      ...
      axios.get(`http://localhost:3344/cartItems/${userId}/${id}`)
        .then((res) => {
          if (res.data.length > 0) {
            const newQuantity = values.quantity + res.data[0].quantity
            axios.patch(`http://localhost:3344/cartItems/${res.data[0].cartItem_id}`, {
            ...
            console.log(res.data)
            axios.post(`http://localhost:3344/cartItems`, {
  ...
  const buyNowForm = useFormik({
    ...
    onSubmit: (values) => {
      ...
      axios.post(`http://localhost:3344/orders`, {
        order_id: orderId,
        date: createDate,
          ...
          <form onSubmit={addToCartForm.handleSubmit}>
            <Flex justify='space-around' align='center' className={styles.productInfoContainer}>
              <span className={styles.imageSection}>
                <Image src={currentProduct.image_url} alt='' width={400} style={{ minWidth: '400px', maxWidth: '400px' }} preview={false} />
              </span>
              <Flex vertical justify='space-around' align='center' style={{ maxHeight: 'min-content', minWidth: '50%' }}>
                ...
                <Flex justify='center' align='center' className={styles.quantitySection}>
                  <Button onClick={decreaseQuantity} style={{ marginRight: "" }}><MinusOutlined /></Button>
                  <InputNumber min={1} max={20} onChange={onQuantityChange} controls={false} value={quantity} size='medium' style={{ width: '17%' }} />
                  <Button onClick={increaseQuantity} style={{ marginLeft: "3%" }}><PlusOutlined /></Button>
                </Flex>
                <Flex gap={30} style={{ margin: '2% 0' }}>
                  <Link className={styles.favoriteSection}><HeartOutlined /> Add to wishlist</Link>
                  <Link className={styles.favoriteSection}><CheckCircleOutlined /> Set an appointment for this</Link>
                </Flex>
          ...
          >
            <form onSubmit={buyNowForm.handleSubmit}>
              <Flex justify='space-between' align='center'>
                <Image src={currentProduct.image_url} alt='' width={200} style={{ minWidth: '200px', maxWidth: '200px' }} />
                <Flex vertical justify='space-between' align='center' style={{ textAlign: 'center' }}>
                  <Text style={{ fontSize: '150%' }}><strong>{currentProduct.name}</strong></Text>
                  ...
                  <Flex style={{ marginTop: '10%' }} gap={10}>
                    ...
                    <Image src={efPointLogo} alt='' width={40} preview={false} style={{ marginBottom: '18%', marginRight: '3%' }} />
                  </Flex>
                </Flex>
              </Flex>
            </form>
